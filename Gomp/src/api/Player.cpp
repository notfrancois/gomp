...
extern "C"
{
	...
	{
		IPlayerPool* players = Gomp::Get()->players;
	...
	{
		IPlayerPool* pool = Gomp::Get()->players;
	...
	GOMPONENT_EXPORT unsigned char player_redirectDownload(void* player, String url)
	{
		...
		{
			Gomp::Get()->core->logLn(LogLevel::Warning, "RedirectDownload can be used only within OnPlayerRequestDownload event.");
			return 0;
		}
		return 1;
	}
